...
> **Picker page** (the human-facing subscribe UI) now lives on the portfolio site at <https://kulenovic.si/calendars/aba-league-calendar/>, maintained in the [`timurkulenovic.github.io`](https://github.com/timurkulenovic/timurkulenovic.github.io) repo. This repo is the **source for the `.ics` feeds** only.
...
- **Picker page:** <https://kulenovic.si/calendars/aba-league-calendar/>
- **All matches:** `webcal://kulenovic.si/aba-league-calendar/ics/all.ics`
- **Per team:** `webcal://kulenovic.si/aba-league-calendar/ics/<team-slug>.ics`
...
Generate subscribable `.ics` feeds into `docs/`:
...
- `ics` renders one `.ics` per team plus `all.ics` into `docs/ics/`. UIDs are stable (`aba-liga-match-<id>@aba-league-calendar`) so re-runs update events in place instead of duplicating them. Timed games use `TZID=Europe/Ljubljana`; games without a published tip-off are all-day.
...
- `.github/workflows/scrape.yml` runs `fetch` + `ics` daily and commits any changes back to `main`, which triggers a GitHub Pages rebuild so the feeds at `kulenovic.si/aba-league-calendar/ics/...` stay fresh.

## Enable GitHub Pages (one-time)

Repo Settings → Pages → Source: **Deploy from a branch** → Branch: `main` / `/docs` → Save. The feeds go live at the `pages_base_url` in `config.toml`.
